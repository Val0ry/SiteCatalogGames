...
if((($_SESSION["user"]["roles"]) != 'user') && (($_SESSION["user"]["roles"]) != 'admin')){
    ...
    if ($_POST){
        if (isset($_POST['id']) && isset($_POST['creator']) && isset($_POST['names']) && isset($_POST['genres']) && isset($_POST['descriptions']) && isset($_POST['notes']) && isset($_POST['links'])){
            ...
            $sql = "UPDATE products SET creator=:creator, names=:names , genres=:genres , descriptions=:descriptions , notes=:notes , links=:links WHERE id = :id";
...
<form method="POST" action="./edGame.php" class="flex flex-col items-center">
    <h1 class="text-5xl py-10 text-center font-extrabold dark:text-white">Edit Game</h1>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mx-32">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed">
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <input class="bg-transparent dark:bg-gray-800" type="text" value="<?= $lineUser['creator'] ?>" name="creator" required>
                            </td>

                            <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <input class="bg-transparent dark:bg-gray-800" type="text" value="<?= $lineUser['names'] ?>" name="names" required>
                            </td>

                            <td class="px-6 py-4">
                            <input class="bg-transparent dark:bg-gray-800" type="text" value="<?= $lineUser['genres'] ?>" name="genres" required>
                            </td>

                            <td class="px-6 py-4">
                            <input class="bg-transparent dark:bg-gray-800" type="textarea" value="<?= $lineUser['descriptions'] ?>" name="descriptions" required>
                            </td>

                            <td class="px-6 py-4">
                            <input class="bg-transparent dark:bg-gray-800" type="number" value="<?= $lineUser['notes'] ?>" name="notes" min="0" max="5" step="0.5" required>
                            </td>

                            <td class="px-6 py-4">
                            <input class="bg-transparent dark:bg-gray-800" type="url" value="<?= $lineUser['links'] ?>" name="links" required>
                            </td>
                            <td class="px-6 py-4">
                            <input class="bg-transparent dark:bg-gray-800" type="url" value="<?= $lineUser['image1'] ?>" name="image1">
                            </td>

                            
                            <td class="px-6 py-4 flex flex-row">
                                <!-- <?php echo '<img id="avatar" src="' .$lineUser['image1']. '" alt="image" title="image"/></img>'; ?> -->
    ...
    <div class="text-center w-32 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">
        <input type="submit" value="EDIT" id="EDIT" onclick="return confirm('Confirm change ?');">
    </div>           
    <div class="text-center w-32 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">
        <a href="./delete.php?id=<?= $lineUser['id'] ?>" onclick="return confirm('Confirm deletion ?'); ">Delete</a>
    </div>           
